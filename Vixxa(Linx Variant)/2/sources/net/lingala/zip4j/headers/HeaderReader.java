...
public class HeaderReader {
    ...
    private CentralDirectory readCentralDirectory(RandomAccessFile randomAccessFile, RawIO rawIO2, Charset charset) throws IOException {
        ...
        while (((long) i3) < numberOfEntriesInCentralDirectory) {
            ...
            byte[] bArr3 = bArr2;
            if (((long) rawIO3.readIntLittleEndian(randomAccessFile2)) == HeaderSignature.CENTRAL_DIRECTORY.getValue()) {
                ...
                if (readShortLittleEndian > 0) {
                    ...
                    fileHeader.setFileName(HeaderUtil.decodeStringWithCharset(bArr7, fileHeader.isFileNameUTF8Encoded(), charset2));
                    ...
                    readAesExtraDataRecord((AbstractFileHeader) fileHeader, rawIO3);
        ...
        if (((long) rawIO3.readIntLittleEndian(randomAccessFile2)) == HeaderSignature.DIGITAL_SIGNATURE.getValue()) {
    ...
    private List<ExtraDataRecord> parseExtraDataRecords(byte[] bArr, int i) {
        ...
        while (i2 < i) {
            ...
            int readShortLittleEndian = this.rawIO.readShortLittleEndian(bArr, i2 + 2);
    ...
    private Zip64ExtendedInfo readZip64ExtendedInfo(List<ExtraDataRecord> list, RawIO rawIO2, long j, long j2, long j3, int i) {
        for (ExtraDataRecord next : list) {
            if (next != null && HeaderSignature.ZIP64_EXTRA_FIELD_SIGNATURE.getValue() == next.getHeader()) {
    ...
    public LocalFileHeader readLocalFileHeader(InputStream inputStream, Charset charset) throws IOException {
        ...
        if (((long) readIntLittleEndian) != HeaderSignature.LOCAL_FILE_HEADER.getValue()) {
            return null;
        }
        ...
        if (Zip4jUtil.readFully(inputStream, bArr2) == 2) {
            ...
            if (readShortLittleEndian > 0) {
                ...
                if (!decodeStringWithCharset.endsWith(InternalZipConstants.ZIP_FILE_SEPARATOR) && !decodeStringWithCharset.endsWith("\\")) {
                    z = false;
                }
                ...
                readAesExtraDataRecord((AbstractFileHeader) localFileHeader, this.rawIO);
    ...
    private void readAesExtraDataRecord(AbstractFileHeader abstractFileHeader, RawIO rawIO2) throws ZipException {
        AESExtraDataRecord readAesExtraDataRecord;
        if (abstractFileHeader.getExtraDataRecords() != null && abstractFileHeader.getExtraDataRecords().size() > 0 && (readAesExtraDataRecord = readAesExtraDataRecord(abstractFileHeader.getExtraDataRecords(), rawIO2)) != null) {
    ...
    private AESExtraDataRecord readAesExtraDataRecord(List<ExtraDataRecord> list, RawIO rawIO2) throws ZipException {
        ...
        for (ExtraDataRecord next : list) {
            if (next != null && next.getHeader() == HeaderSignature.AES_EXTRA_DATA_RECORD.getValue()) {
                ...
                aESExtraDataRecord.setAesVersion(AesVersion.getFromVersionNumber(rawIO2.readShortLittleEndian(data2, 0)));
                byte[] bArr = new byte[2];
                System.arraycopy(data2, 2, bArr, 0, 2);
                aESExtraDataRecord.setVendorID(new String(bArr));
                aESExtraDataRecord.setAesKeyStrength(AesKeyStrength.getAesKeyStrengthFromRawCode(data2[4] & UByte.MAX_VALUE));
                aESExtraDataRecord.setCompressionMethod(CompressionMethod.getCompressionMethodFromCode(rawIO2.readShortLittleEndian(data2, 5)));
