...
public class ExtractAllFilesTask extends AbstractExtractFileTask<ExtractAllFilesTaskParameters> {
    ...
    public void executeTask(ExtractAllFilesTaskParameters extractAllFilesTaskParameters, ProgressMonitor progressMonitor) throws IOException {
        ZipInputStream prepareZipInputStream;
        try {
            prepareZipInputStream = prepareZipInputStream(extractAllFilesTaskParameters.zip4jConfig);
            for (FileHeader next : getZipModel().getCentralDirectory().getFileHeaders()) {
                if (next.getFileName().startsWith("__MACOSX")) {
                    progressMonitor.updateWorkCompleted(next.getUncompressedSize());
                } else {
                    this.splitInputStream.prepareExtractionForFileHeader(next);
                    ZipInputStream zipInputStream = prepareZipInputStream;
                    extractFile(zipInputStream, next, extractAllFilesTaskParameters.outputPath, (String) null, progressMonitor, new byte[extractAllFilesTaskParameters.zip4jConfig.getBufferSize()]);
