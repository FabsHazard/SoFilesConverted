...
public final class MailTo {
    ...
    public static MailTo parse(String str) throws ParseException {
        ...
        if (isMailTo(str)) {
            ...
            if (indexOf2 == -1) {
                str3 = Uri.decode(str.substring(MAILTO_SCHEME.length()));
                str2 = null;
            } else {
                str3 = Uri.decode(str.substring(MAILTO_SCHEME.length(), indexOf2));
                str2 = str.substring(indexOf2 + 1);
            }
            MailTo mailTo = new MailTo();
            if (str2 != null) {
                for (String split : str2.split(ContainerUtils.FIELD_DELIMITER)) {
                    String[] split2 = split.split(ContainerUtils.KEY_VALUE_DELIMITER, 2);
                    if (split2.length != 0) {
