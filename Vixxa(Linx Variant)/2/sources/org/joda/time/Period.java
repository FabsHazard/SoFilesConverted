...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
    ...
    public Weeks toStandardWeeks() {
        ...
        return Weeks.weeks(FieldUtils.safeToInt(((long) getWeeks()) + (((((((long) getMillis()) + (((long) getSeconds()) * 1000)) + (((long) getMinutes()) * TimeUtils.oneMinuteMillis)) + (((long) getHours()) * 3600000)) + (((long) getDays()) * TimeUtils.oneDayMillis)) / 604800000)));
    }

    public Days toStandardDays() {
        checkYearsAndMonths("Days");
        return Days.days(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd((((((long) getMillis()) + (((long) getSeconds()) * 1000)) + (((long) getMinutes()) * TimeUtils.oneMinuteMillis)) + (((long) getHours()) * 3600000)) / TimeUtils.oneDayMillis, (long) getDays()), ((long) getWeeks()) * 7)));
    }

    public Hours toStandardHours() {
        checkYearsAndMonths("Hours");
        return Hours.hours(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(((((long) getMillis()) + (((long) getSeconds()) * 1000)) + (((long) getMinutes()) * TimeUtils.oneMinuteMillis)) / 3600000, (long) getHours()), ((long) getDays()) * 24), ((long) getWeeks()) * 168)));
    }

    public Minutes toStandardMinutes() {
        checkYearsAndMonths("Minutes");
        return Minutes.minutes(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd((((long) getMillis()) + (((long) getSeconds()) * 1000)) / TimeUtils.oneMinuteMillis, (long) getMinutes()), ((long) getHours()) * 60), ((long) getDays()) * 1440), ((long) getWeeks()) * 10080)));
    }

    public Seconds toStandardSeconds() {
        checkYearsAndMonths("Seconds");
        return Seconds.seconds(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd((long) (getMillis() / 1000), (long) getSeconds()), ((long) getMinutes()) * 60), ((long) getHours()) * TimeUtils.oneHourSeconds), ((long) getDays()) * TimeUtils.oneDaySeconds), ((long) getWeeks()) * 604800)));
    }

    public Duration toStandardDuration() {
        checkYearsAndMonths("Duration");
        return new Duration(((long) getMillis()) + (((long) getSeconds()) * 1000) + (((long) getMinutes()) * TimeUtils.oneMinuteMillis) + (((long) getHours()) * 3600000) + (((long) getDays()) * TimeUtils.oneDayMillis) + (((long) getWeeks()) * 604800000));
    }
    ...
    public Period normalizedStandard(PeriodType periodType) {
        PeriodType periodType2 = DateTimeUtils.getPeriodType(periodType);
        Period period = new Period(((long) getMillis()) + (((long) getSeconds()) * 1000) + (((long) getMinutes()) * TimeUtils.oneMinuteMillis) + (((long) getHours()) * 3600000) + (((long) getDays()) * TimeUtils.oneDayMillis) + (((long) getWeeks()) * 604800000), periodType2, (Chronology) ISOChronology.getInstanceUTC());
        int years = getYears();
