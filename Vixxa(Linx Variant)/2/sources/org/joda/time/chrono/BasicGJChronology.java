...
abstract class BasicGJChronology extends BasicChronology {
    ...
    static {
        ...
        long j2 = 0;
        while (i < 11) {
            j += ((long) MIN_DAYS_PER_MONTH_ARRAY[i]) * TimeUtils.oneDayMillis;
            int i2 = i + 1;
            MIN_TOTAL_MILLIS_BY_MONTH_ARRAY[i2] = j;
            j2 += ((long) MAX_DAYS_PER_MONTH_ARRAY[i]) * TimeUtils.oneDayMillis;
    ...
    public long getYearDifference(long j, long j2) {
        ...
        if (yearMillis2 >= FEB_29) {
            if (isLeapYear(year2)) {
                if (!isLeapYear(year)) {
                    yearMillis2 -= TimeUtils.oneDayMillis;
                }
            } else if (yearMillis >= FEB_29 && isLeapYear(year)) {
                yearMillis -= TimeUtils.oneDayMillis;
            }
        }
        int i = year - year2;
