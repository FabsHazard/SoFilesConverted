...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>>> {
        ...
        if (action != 0) {
            ...
            if (action == 1) {
                ...
                if ((Math.abs(xVelocity) > ((float) Utils.getMinimumFlingVelocity()) || Math.abs(yVelocity) > ((float) Utils.getMinimumFlingVelocity())) && this.mTouchMode == 1 && ((BarLineChartBase) this.mChart).isDragDecelerationEnabled()) {
                    ...
                    Log.d("MyChart", "mDecelerationVelocity.x=" + this.mDecelerationVelocity.x + " mLastGesture=" + this.mLastGesture);
                }
                ...
                Log.d("MyChart", "endaction event=" + motionEvent);
                ...
                } else if (((BarLineChartBase) this.mChart).isDragEnabled() && (!((BarLineChartBase) this.mChart).isFullyZoomedOut() || !((BarLineChartBase) this.mChart).hasNoDragOffset())) {
    ...
    public boolean onDoubleTap(MotionEvent motionEvent) {
        ...
        if (((BarLineChartBase) this.mChart).isDoubleTapToZoomEnabled() && ((BarLineScatterCandleBubbleData) ((BarLineChartBase) this.mChart).getData()).getEntryCount() > 0) {
            ...
            ((BarLineChartBase) this.mChart).zoom(f2, f, trans.x, trans.y);
    ...
    public void computeScroll() {
        ...
        if (this.mDecelerationVelocity.x != 0.0f || this.mDecelerationVelocity.y != 0.0f) {
            ...
            float f5 = ((BarLineChartBase) this.mChart).isDragXEnabled() ? this.mDecelerationCurrentPoint.x - this.mTouchStartPoint.x : 0.0f;
            if (((BarLineChartBase) this.mChart).isDragYEnabled()) {
                f = this.mDecelerationCurrentPoint.y - this.mTouchStartPoint.y;
            }
            ...
            Log.d("MyChart", "computeScroll 结束");
            ((BarLineChartBase) this.mChart).calculateOffsets();
            ((BarLineChartBase) this.mChart).postInvalidate();
            stopDeceleration();
            OnChartGestureListener onChartGestureListener = ((BarLineChartBase) this.mChart).getOnChartGestureListener();
            if (onChartGestureListener != null) {
                onChartGestureListener.onChartGestureEnd(obtain, ChartTouchListener.ChartGesture.FLING);
            }
        }
    }
}
