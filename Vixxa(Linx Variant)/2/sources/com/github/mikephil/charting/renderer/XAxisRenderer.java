...
public class XAxisRenderer extends AxisRenderer {
    ...
    public void drawLabel(Canvas canvas, String str, float f, float f2, MPPointF mPPointF, float f3) {
        ...
        if (split.length > 1) {
            ...
            int indexOf = split[1].indexOf(123) + 1;
            int lastIndexOf = split[1].lastIndexOf(125);
            if (indexOf <= 0 || lastIndexOf <= indexOf) {
                f5 = f;
                f4 = f2;
            } else {
                String substring = split[1].substring(indexOf, lastIndexOf);
                split[1] = split[1].substring(lastIndexOf + 1);
                float offsetLeft = this.mViewPortHandler.offsetLeft() + (((float) Utils.calcTextWidth(this.mAxisLabelPaint, split[1])) / 2.0f);
                if (!substring.contains("f") && f >= offsetLeft) {
                    offsetLeft = f;
                }
                f4 = substring.contains("+") ? this.mAxisLabelPaint.getTextSize() + f2 : substring.contains("-") ? f2 - this.mAxisLabelPaint.getTextSize() : f2;
                f5 = offsetLeft;
            }
            Utils.drawXAxisValue(canvas, split[1], f5, f4, paint, mPPointF, f3);
            return;
        }
