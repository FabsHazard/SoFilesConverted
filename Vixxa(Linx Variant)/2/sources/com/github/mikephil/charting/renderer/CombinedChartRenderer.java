...
public class CombinedChartRenderer extends DataRenderer {
    ...
    public void drawValues(Canvas canvas) {
        ...
        for (DataRenderer next : this.mRenderers) {
            if (next instanceof LineChartRenderer) {
                CombinedChart combinedChart = (CombinedChart) this.mChart.get();
                if (combinedChart != null) {
                    if (combinedChart.needDrawLineDataSetValuesAndIcons) {
                        next.drawValues(canvas);
                        list = ((LineChartRenderer) next).getPositions();
                    }
                } else {
                    return;
                }
                ...
                ((ScatterChartRenderer) next).drawValues(canvas, list);
            } else {
                next.drawValues(canvas);
            }
        }
