...
public abstract class Utils {
    ...
    private static final int[] POW_10 = {1, 10, 100, 1000, 10000, BuglyStrategy.a.MAX_USERDATA_VALUE_LENGTH, DurationKt.NANOS_IN_MILLIS, ExceptionCode.CRASH_EXCEPTION, 100000000, Http2Connection.DEGRADED_PONG_TIMEOUT_NS};
    ...
    public static void drawXAxisValue(Canvas canvas, String str, float f, float f2, Paint paint, MPPointF mPPointF, float f3) {
        ...
        float f4 = 0.0f - ((float) rect.left);
        float f5 = (-fontMetrics.ascent) + 0.0f;
        Paint.Align textAlign = paint.getTextAlign();
        paint.setTextAlign(Paint.Align.LEFT);
        if (f3 != 0.0f) {
            float width = f4 - (((float) rect.width()) * 0.5f);
            float f6 = f5 - (fontMetrics2 * 0.5f);
            if (!(mPPointF.x == 0.5f && mPPointF.y == 0.5f)) {
                FSize sizeOfRotatedRectangleByDegrees = getSizeOfRotatedRectangleByDegrees((float) rect.width(), fontMetrics2, f3);
            ...
            if (!(mPPointF.x == 0.0f && mPPointF.y == 0.0f)) {
                f4 -= ((float) rect.width()) * mPPointF.x;
                f5 -= fontMetrics2 * mPPointF.y;
            }
