...
public class LineDataSet extends LineRadarDataSet<Entry> implements ILineDataSet {
    private List<Float> mCircleColorRanges = null;
    ...
    public boolean checkBreak(Entry entry, Entry entry2) {
        return false;
    }
    ...
    public int getCircleColor(int i) {
        ...
        List<Integer> list2 = this.mCircleColors;
        return list2.get(i % list2.size()).intValue();
    }

    public int getCircleColorIndex(Entry entry) {
        if (this.mCircleColorRanges == null) {
            return getEntryIndex(entry);
        }
        float y = entry.getY();
        Iterator<Float> it = this.mCircleColorRanges.iterator();
        int i = 0;
        while (it.hasNext() && y <= it.next().floatValue()) {
            i++;
        }
        return i;
    }
    ...
    public void setCircleColorRanges(List<Float> list) {
        this.mCircleColorRanges = list;
    }
