...
class PagesLoader {
    ...
    private Holder getPageAndCoordsByOffset(float f, boolean z) {
        ...
        if (this.pdfView.isSwipeVertical()) {
            ...
            f2 = Math.abs(f4 - ((this.scaledHeight + this.scaledSpacingPx) * ((float) holder.page))) / this.rowHeight;
            f3 = this.xOffset / this.colWidth;
        } else {
            holder.page = MathUtils.floor(f4 / (this.scaledWidth + this.scaledSpacingPx));
            f3 = Math.abs(f4 - ((this.scaledWidth + this.scaledSpacingPx) * ((float) holder.page))) / this.colWidth;
    ...
    private void loadThumbnail(int i, int i2) {
        if (!this.pdfView.cacheManager.containsThumbnail(i, i2, (float) this.thumbnailWidth, (float) this.thumbnailHeight, this.thumbnailRect)) {
            this.pdfView.renderingHandler.addRenderingTask(i, i2, (float) this.thumbnailWidth, (float) this.thumbnailHeight, this.thumbnailRect, true, 0, this.pdfView.isBestQuality(), this.pdfView.isAnnotationRendering());
        }
    }
    ...
    private int loadRelative(int r11, int r12, boolean r13) {
        ...
        L_0x0033:
            ...
            float r11 = r11 - r13
            float r11 = r11 - r0
            com.github.barteksc.pdfviewer.PagesLoader$Holder r11 = r10.getPageAndCoordsByOffset(r11, r1)
            int r13 = r11.page
        ...
        L_0x0043:
            ...
            int r0 = r0 + -1
            int r0 = com.github.barteksc.pdfviewer.util.MathUtils.min((int) r0, (int) r1)
    ...
    public int loadVisible() {
        ...
        int i3;
        if (this.pdfView.isSwipeVertical()) {
            holder = getPageAndCoordsByOffset(this.pdfView.getCurrentYOffset(), false);
            Holder pageAndCoordsByOffset = getPageAndCoordsByOffset((this.pdfView.getCurrentYOffset() - ((float) this.pdfView.getHeight())) + 1.0f, true);
            if (holder.page == pageAndCoordsByOffset.page) {
                i3 = (pageAndCoordsByOffset.row - holder.row) + 1;
            } else {
                int intValue = ((Integer) this.colsRows.second).intValue() - holder.row;
                for (int i4 = holder.page + 1; i4 < pageAndCoordsByOffset.page; i4++) {
            ...
            holder = getPageAndCoordsByOffset(this.pdfView.getCurrentXOffset(), false);
            Holder pageAndCoordsByOffset2 = getPageAndCoordsByOffset((this.pdfView.getCurrentXOffset() - ((float) this.pdfView.getWidth())) + 1.0f, true);
            if (holder.page == pageAndCoordsByOffset2.page) {
                i2 = (pageAndCoordsByOffset2.col - holder.col) + 1;
            } else {
                int intValue2 = ((Integer) this.colsRows.first).intValue() - holder.col;
                for (int i6 = holder.page + 1; i6 < pageAndCoordsByOffset2.page; i6++) {
    ...
    public void loadPages() {
        ...
        float currentScale = pDFView3.toCurrentScale((float) pDFView3.getSpacingPx());
        this.scaledSpacingPx = currentScale;
        this.scaledSpacingPx = currentScale - (currentScale / ((float) this.pdfView.getPageCount()));
