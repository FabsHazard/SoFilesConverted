...
public class HctSolver {
    ...
    static boolean isBounded(double d) {
        return Utils.DOUBLE_EPSILON <= d && d <= 100.0d;
    }
    ...
    static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (nthVertex[0] >= Utils.DOUBLE_EPSILON) {
    ...
    static double inverseChromaticAdaptation(double d) {
        ...
        return ((double) MathUtils.signum(d)) * Math.pow(Math.max(Utils.DOUBLE_EPSILON, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }
