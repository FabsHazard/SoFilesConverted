...
public final class Cam16 {
    ...
    private final double[] tempArray = {Utils.DOUBLE_EPSILON, Utils.DOUBLE_EPSILON, Utils.DOUBLE_EPSILON};
    ...
    static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(d15, d17, pow4, c, flRoot, Math.sqrt((pow5 * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, (1.7000000000000002d * pow4) / ((0.007d * pow4) + 1.0d), log1p * Math.cos(radians), log1p * Math.sin(radians));
    }
    ...
    private static Cam16 fromJchInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        double d5 = d3;
        double d6 = d / 100.0d;
        double c = (4.0d / viewingConditions.getC()) * Math.sqrt(d6) * (viewingConditions.getAw() + 4.0d) * viewingConditions.getFlRoot();
        ...
        return new Cam16(d5, d4, d, c, flRoot, Math.sqrt(((d2 / Math.sqrt(d6)) * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, d7, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
    ...
    public double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] dArr) {
        double pow = Math.pow(((getChroma() == Utils.DOUBLE_EPSILON || getJ() == Utils.DOUBLE_EPSILON) ? 0.0d : getChroma() / Math.sqrt(getJ() / 100.0d)) / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d), 1.1111111111111112d);
        ...
        double signum = Math.signum(d5) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(Utils.DOUBLE_EPSILON, (Math.abs(d5) * 27.13d) / (400.0d - Math.abs(d5))), 2.380952380952381d);
        double signum2 = Math.signum(d6) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(Utils.DOUBLE_EPSILON, (Math.abs(d6) * 27.13d) / (400.0d - Math.abs(d6))), 2.380952380952381d);
        double signum3 = Math.signum(d7) * (100.0d / viewingConditions.getFl()) * Math.pow(Math.max(Utils.DOUBLE_EPSILON, (Math.abs(d7) * 27.13d) / (400.0d - Math.abs(d7))), 2.380952380952381d);
