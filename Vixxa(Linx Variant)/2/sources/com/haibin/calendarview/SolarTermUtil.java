...
public final class SolarTermUtil {
    ...
    private static final double[] E30 = {1.00013988799d, Utils.DOUBLE_EPSILON, Utils.DOUBLE_EPSILON, 0.01670699626d, 3.09846350771d, 6283.0758499914d, 1.3956023E-4d, 3.0552460962d, 12566.1516999828d, 3.08372E-5d, 5.19846674381d, 77713.7714681205d, 1.628461E-5d, 1.17387749012d, 5753.3848848968d, 1.575568E-5d, 2.84685245825d, 7860.4193924392d, 9.24799E-6d, 5.45292234084d, 11506.7697697936d, 5.42444E-6d, 4.56409149777d, 3930.2096962196d};
    private static final double[] E31 = {0.00103018608d, 1.10748969588d, 6283.0758499914d, 1.721238E-5d, 1.06442301418d, 12566.1516999828d, 7.02215E-6d, 3.14159265359d, Utils.DOUBLE_EPSILON};
    ...
    private static double EnnT = Utils.DOUBLE_EPSILON;
    ...
    private static final double[] GXC_L = {4.89506311081711d, 628.3319667475674d, 5.291887161265863E-6d, Utils.DOUBLE_EPSILON, Utils.DOUBLE_EPSILON};
    private static final double[] GXC_P = {1.796595680777785d, 0.030010238356341697d, 8.028514559173916E-6d};
    private static final double[] H_C_ANGLE_TABLE = {Utils.DOUBLE_EPSILON, 50287.92262d, 111.24406d, 0.07699d, -0.23479d, -0.00178d, 1.8E-4d, 1.0E-5d};
    ...
    private static double MnnT = Utils.DOUBLE_EPSILON;
    private static final double SECOND_PER_RAD = 206264.80624709636d;
    private static String[] SOLAR_TERMS;
    private static final double[] nutB = {2.1824391966d, -33.757045954d, 3.62262E-5d, 3.734E-8d, -2.8793E-10d, -171996.0d, -1742.0d, 92025.0d, 89.0d, 3.5069406862d, 1256.663930738d, 1.05845E-5d, 6.9813E-10d, -2.2815E-10d, -13187.0d, -16.0d, 5736.0d, -31.0d, 1.3375032491d, 16799.418221925d, -5.11866E-5d, 6.4626E-8d, -5.3543E-10d, -2274.0d, -2.0d, 977.0d, -5.0d, 4.3648783932d, -67.514091907d, 7.24525E-5d, 7.4681E-8d, -5.7586E-10d, 2062.0d, 2.0d, -895.0d, 5.0d, 0.0431251803d, -628.301955171d, 2.682E-6d, 6.5935E-10d, 5.5705E-11d, -1426.0d, 34.0d, 54.0d, -1.0d, 2.3555557435d, 8328.691425719d, 1.545547E-4d, 2.5033E-7d, -1.1863E-9d, 712.0d, 1.0d, -7.0d, Utils.DOUBLE_EPSILON, 3.4638155059d, 1884.965885909d, 7.9025E-6d, 3.8785E-11d, -2.8386E-10d, -517.0d, 12.0d, 224.0d, -6.0d, 5.4382493597d, 16833.175267879d, -8.74129E-5d, 2.7285E-8d, -2.475E-10d, -386.0d, -4.0d, 200.0d, Utils.DOUBLE_EPSILON, 3.6930589926d, 25128.109647645d, 1.033681E-4d, 3.1496E-7d, -1.7218E-9d, -301.0d, Utils.DOUBLE_EPSILON, 129.0d, -1.0d, 3.5500658664d, 628.361975567d, 1.32664E-5d, 1.3575E-9d, -1.7245E-10d, 217.0d, -5.0d, -95.0d, 3.0d};
    ...
    private static double worldTimeDiff(double d) {
        ...
        double[] dArr = DTS;
        double d2 = dArr[i];
        double d3 = ((d - d2) / (dArr[i + 5] - d2)) * 10.0d;
        double d4 = d3 * d3;
        return dArr[i + 1] + (dArr[i + 2] * d3) + (dArr[i + 3] * d4) + (dArr[i + 4] * d4 * d3);
    }
    ...
    private static void addGxc(double d, double[] dArr) {
        ...
        double d9 = d5 - d8;
        double d10 = d6 - d8;
        dArr[0] = d8 - (((Math.cos(d9) - (Math.cos(d10) * d7)) * GXC_K) / Math.cos(dArr[1]));
        double d11 = dArr[1];
        dArr[1] = d11 - ((Math.sin(d11) * GXC_K) * (Math.sin(d9) - (d7 * Math.sin(d10))));
        dArr[0] = rad2mrad(dArr[0]);
    }

    private static Nutation nutation(double d) {
        Nutation nutation = new Nutation();
        double unused = nutation.Lon = Utils.DOUBLE_EPSILON;
        double unused2 = nutation.Obl = Utils.DOUBLE_EPSILON;
        double d2 = d / 36525.0d;
        double d3 = d2 * d2;
        double d4 = d3 * d2;
        double d5 = d4 * d2;
    ...
    private static double[] moonCoord(double d) {
        ...
        double d6 = dArr[0];
        double[] dArr2 = M1n;
        double d7 = d6 + dArr2[0] + (dArr2[1] * d3) + (dArr2[2] * d4) + (dArr2[3] * d5) + (dArr2[4] * d5 * d3);
        dArr[0] = d7;
        dArr[0] = rad2mrad(d7);
        precession(d2, dArr);
        return dArr;
    ...
    private static double angleDiff(int i, double d, double d2) {
        ...
        double access$800 = earCal[0] + nutation(d).Lon;
        earCal[0] = access$800;
        return rad2mrad(d2 - access$800);
    }
    ...
    public static String[] getSolarTerms(int i) {
        ...
        for (int i3 = 0; i3 < 19; i3++) {
            strArr[i3 + 3] = setFromJulian(getTimeFromAngle((((double) i3) * 15.2d) + d, (double) (i3 * 15), 0) + J2000 + 0.3333333333333333d, true).toString() + SOLAR_TERMS[i3];
        }
        return strArr;
    }
    ...
    private static String[] getSolarTermsPreOffset(int i) {
        ...
        for (int i2 = 21; i2 < 24; i2++) {
            strArr[i2 - 21] = setFromJulian(getTimeFromAngle((((double) i2) * 15.2d) + d, (double) (i2 * 15), 0) + J2000 + 0.3333333333333333d, true).toString() + SOLAR_TERMS[i2];
        }
        return strArr;
    }
    ...
    private static String[] getSolarTermsNextOffset(int i) {
        ...
        for (int i2 = 19; i2 < 21; i2++) {
            strArr[i2 - 19] = setFromJulian(getTimeFromAngle((((double) i2) * 15.2d) + d, (double) (i2 * 15), 0) + J2000 + 0.3333333333333333d, true).toString() + SOLAR_TERMS[i2];
        }
        return strArr;
    }
    ...
    public static String doubleToString(double d) {
        int i = (int) d;
        return d < 10.0d ? "0" + i : String.valueOf(i);
    }
}
